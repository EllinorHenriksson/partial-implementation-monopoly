...
public class Player {
  ...
  private ConsoleUi.Action action;
  ...
  /**
   * Handles the interaction when it is the player's turn to perform some actions.
   */
  public boolean yourTurn(Dice d1, Dice d2) {    
    ...
    return action == ConsoleUi.Action.ROLL;  // if we have rolled we are done
  }
  ...
  /**
   * Gets the console user interface (property ui) of the player.
   *
   * @return The ui.
   */
  ...
  /**
   * Sets the value of the action property of the player.
   *
   * @param a - The action.
   */
  public void setAction(ConsoleUi.Action a) {
    action = a;
  }
}
